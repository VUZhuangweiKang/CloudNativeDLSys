...
class DLCJobDataset(Dataset[T_co], metaclass=abc.ABCMeta):
    def __init__(self, name: str):
        ...
        # every file is a block when probing, so the num of blocks is same as the num of files
        self.is_probing = self.num_samples == self.num_blocks
        self.cache_hits = 0
        self.io_times, self.processing_times = [], []
        
        self.num_unused_items = {}  # the number of unused items in a block
        self.opened_blocks = {}
        self.block_keys = {}
    ...
    def try_get_item(self, block_path, block_idx, sample_idx):
        if self.is_probing:
            if block_path.endswith('.npy'):
                data = np.load(block_path, allow_pickle=True)
            elif block_path.endswith('.pkl'):
                with open(block_path, 'rb') as f:
                    data = pickle.load(f)
            elif block_path.endswith('.pt'):
                data = torch.load(f)
            else:
                with open(block_path, 'rb') as f:
                    data = f.read()
            return block_path, data
        else:
            if block_idx not in self.opened_blocks:
                self.opened_blocks[block_idx] = h5py.File(block_path, 'r')
                self.block_keys[block_idx] = self.opened_blocks[block_idx]['keys'][:]
                self.num_unused_items[block_idx] = len(self.block_keys[block_idx])

            # key = self.block_keys[block_idx][sample_idx]
            # val = self.opened_blocks[block_idx][key][:]
            
            key = self.block_keys[block_idx][sample_idx]
            val = self.opened_blocks[block_idx]['values'][sample_idx]
            
            # if block_path.endswith(".bin"):
            #     data = data.tobytes()
            return key.decode().replace('|' ,'/'), val
    ...
    def __getitem__(self, index: Tuple):
        ...
        try:
            file_name, data = self.try_get_item(block_path, block_idx, item_idx)
            self.cache_hits += 1
        ...
        if not self.is_probing:
            ...
            if self.num_unused_items[block_idx] == 0:
                self.opened_blocks[block_idx].close()
                del self.opened_blocks[block_idx]
                del self.block_keys[block_idx]
                del self.num_unused_items[block_idx]
        self.io_times.append(time.time() - start)
        
        start = time.time()
        item = self._process_item(item_cloud_path=f"{chunk_cloud_path}/{file_name.split('/')[-1]}", contents=data)
...
class _DLCJobDataLoaderIter(_BaseDataLoaderIter):
    def __init__(self, loader, num_batches, opt_num_workers, autoscale_workers: bool = False, tune_iters: int = None,
        ...
        self._socket_req.send_multipart([b"init", self._dataset._name.encode('utf-8'), pickle.dumps(batches)])
    ...
    def _next_data(self):
        ...
        self._data_load_time.append(self._last_iter_time - start)
        return data
