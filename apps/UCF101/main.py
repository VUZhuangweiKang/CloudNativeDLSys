...
def split_dataloader(train_data, validation_split = 0.2):
    ...
    train_data = DLCJobDataLoader(dataset, batch_size=batch_size, sampler = train_sampler, num_workers = num_workers, drop_last = False)
    val_data = DLCJobDataLoader(dataset, batch_size=batch_size, sampler = val_sampler, num_workers = num_workers, drop_last = False)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    hidden_size = 128
    num_lstm_layers = 2
    model = ClassificationModel(num_classes = num_classes, hidden_size = hidden_size, num_lstm_layers = num_lstm_layers, backbone_name = 'resnet101')
    ...
    train_dataset = VideoDataset(name='train', trainTest_dir='./ucfTrainTestlist', num_frames = num_frames, transform = transform)
    ...
    train_dataloader = DLCJobDataLoader(train_dataset, shuffle = False, batch_size=args.batch_size, drop_last = False, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        dur = train(model=model, train_loader=train_dataloader, criterion=criterion, optimizer=optimizer, epoch=epoch, args=args)
        summary.append(dur)
